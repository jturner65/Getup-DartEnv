...
class skelHolder(ABC):   
    ...
    #kneeDOFIdxs : dof idxs of knee joints.  for possible reward function
...
#            pass
    ...
    def setDesiredExtAssist(self, desFrcTrqVal, desExtFrcVal_mults, desExtFrcVal_dir, setReciprocal, obsUseMultNotFrc):
...
#        else :
    ...
    #this will return the assistive force component currently being used as the final part of the observation
    def getObsAssist(self):
...
class ANASkelHolder(skelHolder):#, ABC):
    ...
    rwdNames = ['eefDist','action', 'height','footMovDist','lFootMovDist','rFootMovDist','comcop','contacts','UP_COMVEL','X_COMVEL','Z_COMVEL','GAE_getUp','kneeAction','matchGoalPose','assistFrcPen','eefDistNeg']
    ...
    def setRwdsToUse(self, rwdFuncsToUse):
        ...
        wtVals = defaultdict(lambda:1.0,{'eefDist':5.0,'eefDistNeg':5.0,'matchGoalPose':10.0,'action':1.0,'height':100.0,'footMovDist':10.0,'lFootMovDist':10.0,'rFootMovDist':10.0,'UP_COMVEL':10.0, 'comcop':20.0, 'contacts':10})
    ...
    def setRwdWtsVars(self, names, wts, varVals, tolVals, rwdsToUse):
        ...
        self.rwdFuncEvals = {
            ...
            'eefDistNeg':self.calcRwdVal_EefDistNeg,
...
#            self.initPose = np.copy(self.skel.q) 
    ...
    #calculate all force and torque components for current state of skel -TODO fix this, not working
...
#        eefPull = JtPullPInv_new.dot(eefTau) #- self.desExtFrcVal
#        eefPullNoCntct = JtPullPInv_new.dot(eefTauNoCntct) #- self.desExtFrcVal
        #component of eefPullFrc tangent to assist force
#        assistDir= self.desExtFrcDir
#        #use negative assist dir since we want the reactive-force component
#        eefPullTan =  eefPull.dot(assistDir) * assistDir
#        eefPullOrtho = eefPull - eefPullTan
#        self.orthoCompensateEefFrc = -eefPullOrtho#-eefPullOrtho
...
#        print('eefPull force : {} assist dir : {} assist Frc : {} eef in assist dir : {} ortho to assist : {} applied : {}'.format(eefPull, assistDir, self.desExtFrcVal, eefPullTan, eefPullOrtho, self.orthoCompensateEefFrc))
    ...
    def calcRwdVal_CnstrntFrcPen(self, name):
        sclFact = self.getRaiseProgress()
        #self.des
        cFrc = self.cnstrntBody.constraint_forces()
        magFrc = np.linalg.norm(cFrc)
        rwdComp = -1.0 * magFrc

        rwdValWatched = [('constraintFrc',cFrc), ('sclFact(Ht Fract)',sclFact)]
        succCond = False
        failCond = magFrc > self.mg
        return rwdComp, rwdValWatched, succCond, failCond
    ...
    def calcRwdVal_GoalPoseMatch(self, name):
        #sclFact is up to 1, may be negative
        sclFact = self.getRaiseProgress()
        ...
        rwdValWatched = [('goalPoseVariance',goalPoseVariance), ('sclFact(Ht Fract)',sclFact)]
    ...
    def calcRwdVal_KneeAction(self, name):
        optVal = self.a[self.kneeDOFIdxs]
        ...
        rwdComp = self.rwdWts[name] * kneeAct

        succCond = False
        failCond = kneeAct < .01    #means both knees are pulling hard - maybe check to see that both knees are pushing?

        rwdValWatched=[('leftKnee_A',self.a[self.kneeDOFIdxs[0]]), ('rightKnee_A',self.a[self.kneeDOFIdxs[1]])]
        return rwdComp, rwdValWatched, succCond, failCond
    ...
    def calcRewardAndCheckDone(self, obs, debug, dbgStructs):
        ...
        if rwdComps['isBadDone']:
            rwdComps['reward'] -= 1.0 #TODO change this to make reward 0?
        elif rwdComps['isGoodDone'] : 
            rwdComps['reward'] += 1000.0 #extra reward for success

        if (debug):
            assistCmp = self.env.getSkelAssistObs(self)
            print('Step : {} : AssistFrc {} : Reward : {:.7} Done : {}'.format(numSteps,assistCmp, rwdComps['reward'], done)),
...
#        self.osc_Kd = np.diag(np.ones(self.ndofs)*Kd)
    ...
    def frwrdStepBotForFrcVal(self, desFrc, desFMult, recip, obsUseMultNotFrc, restoreBotSt=True, dbgFrwrdStep=False):
        ...
        self.debug=False        
        #find dir of force
        norm=np.linalg.norm(desFMult)
        if (norm == 0) :
            desFrcDir = np.array([0,0,0]) 
        else : 
            desFrcDir = (desFMult / norm)
            
        #init and solve for bot optimal control for current target force
        self.setDesiredExtAssist(desFrc, desFMult, desFrcDir, setReciprocal=recip, obsUseMultNotFrc=obsUseMultNotFrc) 
