...
#this environment will use akanksha's trained policy w/constraint doing most of the work
#TODO this needs to be modified to perform this - code has been commented out
class DartStandUp3dAssistEnv_AKCnstrnt(dart_env_2bot.DartEnv2Bot, utils.EzPickle):
    def __init__(self):
        """
        ...
        self.useAK_Kima = True
        #called before skeletons loaded so scales can be set properly
        self.setupUsingOldEnvPol(use=True)
    ...
    def _initAnaKimaSkel(self, skel, skelType, actSclBase,  basePose):           
        #we used 50 for ks to train AK's policy
        ksVal = 50.0
        kdVal = 10.0
        #kima used in AK's trained constrained environment used scale base of 100 - must use old kima skeleton
        actSclBase = 100
        actSclIdxMult = [([1, 2, 7, 8], .1), #thigh twist and spread 
                    ([15,17, 19, 21], .05),#left shoulders, 2 of bicep, and hands
                    ([4,5, 10, 11], .2),#ankles 
                    ([18, 22], .1)] #feet and forearms
...
#                done=True
    ...
    def getRunDirName(self):
        nameStr = self.skelHldrs[self.botIdx].name + '_' + self.datetimeStr 
        nameStr = nameStr.replace(' ','_')
        return nameStr
        
    #return a name for the image
    def getImgName(self):        
        return self.skelHldrs[self.botIdx].name
    ...
    def isLegalAssistVal(self, frcVal, isFrcMult):
