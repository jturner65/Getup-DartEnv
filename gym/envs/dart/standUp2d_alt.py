...
class DartStandUp2dEnv(dart_env.DartEnv, utils.EzPickle):
    ...
    def setPerStepAssistForce(self, xfrc, yfrc):
        self.setROForceVal(xfrc, yfrc, isMult=False)
        self.resetUseSetFrc = False
        self.initXYAssistForce()  
...
#            self._get_viewer().runSingleStep()
    ...
    #set force value multiplier, to be added as assistive force for entire rollot
    #this is only before the beginning of the rollout
    #frcVal is vector of values for force
    #isMult : true means this is mg multiplier, false means actual force
    def setROForceVal(self, frcValX, frcValY, isMult):
        self.resetUseSetFrc = True
        #if is multiplier set multiplier directly
        if not isMult :
            frcValX = self.getMultFromFrc(frcValX)
            frcValY = None if (frcValY is None) else self.getMultFromFrc(frcValY)
        
        if(frcValY==None):
        #if no frcY then frxX is magnitude : hyp of pi/4 force; we want x and y values of magnitude
            xyVal = frcValX/1.41421356237
            self.frcMult[0] = xyVal
            self.frcMult[1] = xyVal
        else :
            self.frcMult[0] = frcValX
            self.frcMult[1] = frcValY
...
#        
    ...
    #cause all future rollouts to reset state with random force
    ...
    def _resetForce(self):
        if(not self.resetUseSetFrc):  #using random force       
