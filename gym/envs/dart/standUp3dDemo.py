...
class DartStandUp3dDemo(dart_env_2bot.DartEnv2Bot, utils.EzPickle):
    def __init__(self):
        #all skeleton init done in DartEnv2Bot
        dart_env_2bot.DartEnv2Bot.__init__(self, 'bipedJT.skel', 1, dt=.002, disableViewer=False)
        ...
        #current active skelHandler (holds target skeleton for policy actions) is self.activeSkelHndlr
        
        #penalty for breaking simulation due to illegal torques causing severe breach.
        #must be negative
        self.brokeSimPenalty = 0
    ...
    def _resetForce(self):
        if(not self.useSetFrc):  #using random force       
            ...
            self.frcMult[2] = self.np_random.uniform(-0.5, 0.5)
