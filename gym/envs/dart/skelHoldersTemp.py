...
#              print('!!!!!!!!!!!!!!!!!!!!!! non zero ground contact y : {}'.format(contact.point))
    ...
    #return the jacobian inverse transpose of this skel at end effector
    def getJInvTrans(self):
        Jtrans = np.transpose(self.reachBody.world_jacobian(offset=self.reachBodyOffset))
        return np.linalg.pinv(Jtrans),Jtrans
...
#        self.osc_Kd = np.diag(np.ones(self.ndofs)*Kd)
    ...
    def frwrdSimBot_DispIKSPD(self, desDisp, dbgFrwrdStep=False):
        ...
        #we need to derive desired dq by taking starting dq and average dq (Which will result in desired eef displacement == (desSPDPose - orig q)/dt )
        avgDq = (qBar - q)/dt
        #avgDq == (dq0 + dq1)/2 -? dq1 = (2*avgDq) - dq0
        dqBar = (2*avgDq) - dq
...
#        input()
        ...
        d = self.zeroKD #-mKd.dot(dq - dqBar)                        
        #d = -mKd.dot(dq - dqBar)                       
